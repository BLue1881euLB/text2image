...
class text2image:
    def __init__(self, alphabet, fontsize=30, imagenum=30, width=300, height=50, color_dist=200,
                 linecnt=3, pointcnt=10, ellipsecnt = 10, ttf='font.ttf', radius=3, c_n=10,
                 font_size=30, rgb=(150, 200, 255), img_dir='./imgs', p_blur=0.4, p_small=0.7, p_erode=0.8):
        ...
        num = [str(_) for _ in range(0, 10)]  # 0~9
        lower = [chr(_) for _ in range(97, 123)]  # 小写字母a~z
        upper = [chr(_) for _ in range(65, 91)]  # 大写字母A~Z
        self.alphabet = self.alphabet + num + lower + upper
        self.choice = self.alphabet.copy()
        # self.label = {}


    def getRandomColor(self, is_bg=True):
        ...
        if is_bg:
            ...
            color_dist = np.linalg.norm(np.array([r, g, b]) - np.array(self.bg_color))
            while color_dist < self.color_dist:
                r = random.randint(0, 255)
                g = random.randint(0, 255)
                b = random.randint(0, 255)
                color_dist = np.linalg.norm(np.array([r, g, b]) - np.array(self.bg_color))
    ...
    # def getRandomChar(self, Hanzi):
    #     num = [str(_) for _ in range(0, 10)]  # 0~9
    #     lower = [chr(_) for _ in range(97, 123)]  # 小写字母a~z
    #     upper = [chr(_) for _ in range(65, 91)]  # 大写字母A~Z
    #     random_char = random.choice(Hanzi + num + lower + upper)
    #     return random_char

    def drawLine(self, draw, width, height):
        for i in range(np.random.randint(self.linecnt)):
            x1 = random.randint(0, width)
            x2 = random.randint(0, width)
            y1 = random.randint(0, height)
            y2 = random.randint(0, height)
            draw.line((x1, y1, x2, y2), fill=self.getRandomColor(is_bg=False))

    def drawPoint(self, draw, width, height):
        for i in range(np.random.randint(self.pointcnt)):
            x = random.randint(0, width)
            y = random.randint(0, height)
            draw.point((x, y), fill=self.getRandomColor(is_bg=False))

    def drawEllipse(self, draw, radius, width, height):
        for i in range(np.random.randint(self.ellipsecnt)):
            x = random.randint(0, width)
            y = random.randint(0, height)
            r_x = random.randint(1, radius)
            r_y = random.randint(1, radius)
            draw.ellipse((x - r_x, y - r_y, x + r_x, y + r_y), outline=self.getRandomColor(is_bg=False))

    def blur(self, image, x_blur_max=3, y_blur_max=3, g_blur_max=3):
        x_blur = np.random.randint(1, x_blur_max)
        y_blur = np.random.randint(1, y_blur_max)
        g_blur = np.random.randint(1, g_blur_max)
        p = np.random.rand(1)
        if p <= 0.5:
            dst = cv2.blur(image, (x_blur, y_blur))
        else:
            # dst = cv2.blur(image, (x_blur, y_blur))
            dst = cv2.GaussianBlur(image, (3, 3), g_blur)
            # dst = cv2.medianBlur(image, m_blur)
        return dst

    def erode_dilate(self, src):
        mask_width = np.random.randint(30, self.width)
        sss = np.zeros([self.height, self.width], dtype=np.uint8)
        x = np.random.randint(0, self.width-mask_width)
        sss[:self.height, x:x+mask_width] = 255
        sss_r = 255 - sss
        x = cv2.getStructuringElement(cv2.MORPH_CROSS, (2, 2))
        # x = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))
        image = cv2.add(src, np.zeros(np.shape(src), dtype=np.uint8), mask=sss)
        # cv2.imshow('sss', image)
        image_r = cv2.add(src, np.zeros(np.shape(src), dtype=np.uint8), mask=sss_r)
        # cv2.imshow('sss_r', image_r)
        p = np.random.rand(1)
        if p < 0.9:
            result1 = cv2.erode(image, x)
        else:
            result1 = cv2.dilate(image, x)
    ...
    def generate(self):
        f = open('{}/label.txt'.format(self.img_dir), 'w', encoding='utf-8')
        for save_i in tqdm(range(self.imagenum)):
            img_path = '{}/{}.png'.format(self.img_dir, save_i)
            # txt_color = getRandomColor(is_bg=False)
            self.bg_color = self.getRandomColor(is_bg=True)

            img = Image.new(mode="RGB", size=(self.width, self.height), color=self.bg_color)
            draw = ImageDraw.Draw(img)
            # charactor
            label = []
            if len(self.choice) < self.c_n:
                self.choice = self.alphabet.copy()
                # print(len(self.alphabet))
                print('Update alphabet!')

            for i in range(self.c_n):
                txt_color = self.getRandomColor(is_bg=False)

                random_txt = random.choice(self.choice)

                self.choice.remove(random_txt)
                label.append(random_txt)
                # p = np.random.rand(1)
                # if p > self.p_small:
                #     self.font = ImageFont.truetype(self.ttf, self.font_size)
                # else:
                #     self.font = ImageFont.truetype(self.ttf, np.random.randint(int(self.fontsize*0.6), int(self.fontsize*0.8)))
                self.font = ImageFont.truetype(self.ttf, self.font_size)

                draw.text((self.font_size * i, 3), text=random_txt, fill=txt_color, font=self.font)
            # line point ellipse
            # self.label[save_i] = label
            f.writelines(img_path + ':' + ','.join(label)+'\n')
            self.drawLine(draw, self.width, self.height)
            self.drawPoint(draw, self.width, self.height)
            self.drawEllipse(draw, self.radius, self.width, self.height)

            cv_img = cv2.cvtColor(np.asarray(img), cv2.COLOR_RGB2BGR)

            p = np.random.rand(1)
            if p < self.p_erode:
                cv_img = self.erode_dilate(cv_img)

            p = np.random.rand(1)
            if p < self.p_blur:
                cv_img = self.blur(image=cv_img)
...
if __name__ == '__main__':
    ...
    test = text2image(alphabet=alphabet, p_blur=0.9, p_erode=0.9, imagenum=100000, img_dir='./img_singal_bkg')
    test.generate()
